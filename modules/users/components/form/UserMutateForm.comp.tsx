'use client'

import Field from '@/components/form/Field'
import { Button } from '@/components/ui/button'
import { Form } from '@/components/ui/form'
import {
	useUserCreate,
	useUserListQuery,
	useUserUpdate,
} from '@/modules/users/hooks'
import { UserModel } from '@/modules/users/types'
import { TableWithTimestampsModel } from '@/types'
import { zodResolver } from '@hookform/resolvers/zod'
import { useRouter } from 'next/navigation'
import { useForm } from 'react-hook-form'
import { toast } from 'sonner'
import { z } from 'zod'

const FormSchema = z.object({
	name: z.string(),
	phone: z.string().optional(),
	email: z.coerce.string().optional(),
})

type UserMutateFormProps = Partial<Pick<UserModel, 'id'>>

export function UserMutateForm({ id }: UserMutateFormProps) {
	const { data: users } = useUserListQuery({
		id,
	})
	const userCreate = useUserCreate({
		onSuccess: () => {
			toast('User Created', {
				description: 'User has been created successfully.',
			})
		},
		onError: (error) => {
			toast('Error', {
				description: error?.message,
			})
		},
	})
	const userUpdate = useUserUpdate({
		onSuccess: () => {
			toast('User Updated', {
				description: 'User has been updated successfully.',
			})
		},
		onError: (error) => {
			toast('Error', {
				description: error?.message,
			})
		},
	})
	const router = useRouter()
	const user = users?.find((u) => u.id === id)

	const form = useForm<z.infer<typeof FormSchema>>({
		resolver: zodResolver(FormSchema),
		defaultValues: {
			name: user?.name || '',
			phone: user?.phone || '',
			email: user?.email || '',
		},
	})

	function handleCreateUser(
		user: Omit<UserModel, keyof TableWithTimestampsModel>
	) {
		userCreate.mutate({
			...user,
		})
		return {
			success: 1,
			message: 'User has been created successfully.',
		}
	}

	function handleUpdateUser(
		user: Partial<Omit<UserModel, keyof TableWithTimestampsModel>>
	) {
		if (!id) {
			return
		}
		userUpdate.mutate(id, {
			...user,
		})

		return {
			success: 1,
			message: 'User has been updated successfully.',
		}
	}

	function onSubmit(data: z.infer<typeof FormSchema>) {
        console.log({data})
		const result = id
			? handleUpdateUser(data)
			: handleCreateUser(data)
		if (result?.success) {
			// toast('Success', {
			// 	description: result.message,
			// 	action: {
			// 		label: 'Ok',
			// 		onClick: () => {},
			// 	},
			// })
			router.push('/users')
		}
	}

	return (
		<Form {...form}>
			<form
				onSubmit={form.handleSubmit(onSubmit)}
				className="w-2/3 space-y-6"
			>
				<Field
					name="name"
					type="text"
					label="Name"
					placeholder="Enter the user's full name"
					min={1}
					form={form}
					required
				/>

				<Field
					name="phone"
					type="text"
					label="Phone"
					placeholder="Enter the phone"
					form={form}
				/>

				<Field
					name="email"
					type="text"
					label="Email"
					placeholder="Enter the email"
					form={form}
				/>

				<Button variant="success" type="submit">
					{id ? 'Update' : 'Create'} new user
				</Button>
			</form>
		</Form>
	)
}
